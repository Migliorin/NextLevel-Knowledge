...
export class AuthService {
  ...
  private normalizeName(name: string): string {
    ...
    try {
      return Buffer.from(trimmedName, 'latin1')
        .toString('utf8')
        .normalize('NFC');
    } catch {
      return trimmedName.normalize('NFC');
  ...
  async refresh(
    ...
    await this.blacklistPreviousAccessToken(authorization, existUser.id);
  ...
  async logout(userId: number, authorization?: string): Promise<void> {
    await this.blacklistPreviousAccessToken(authorization, userId);
  ...
  private async blacklistPreviousAccessToken(
    ...
    const ttlSeconds = accessPayload.exp - nowInSeconds;

    if (ttlSeconds <= 0) {
      return;
    }

    await this.redisService.blacklistAccessToken(accessToken, ttlSeconds);
  }
